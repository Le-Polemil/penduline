...
/**
 * ⚠️ DOUBLON ASSUMÉ de `apps/web/src/data/useTitreDepliable.ts` — les deux
 * fichiers sont IDENTIQUES à ce préambule près, et doivent le rester.
 *
 * Le partager supposerait de faire entrer React dans `@penduline/shared`, qui est
 * de la logique pure sans dépendance de rendu ; et il n'y a ici aucune logique
 * pure à extraire — tout est mesure du DOM et désambiguïsation d'événements. Même
 * arbitrage que `Icons.tsx`, et c'est #79 qui tranchera la forme d'un éventuel
 * paquet de composants partagés.
 *
 * L'option `doubleClic` existe précisément pour que les deux copies restent
 * identiques malgré leur seule divergence : le panneau ne renomme pas au
 * double-clic, il n'a donc rien à désambiguïser.
 */
